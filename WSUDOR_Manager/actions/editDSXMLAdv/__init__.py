...
editDSXMLAdv = Blueprint('editDSXMLAdv', __name__, template_folder='templates', static_folder="static")
...
@editDSXMLAdv.route('/editDSXMLAdv', methods=['POST', 'GET'])
...
def index():
    ...
    PIDlet['pURL'] = "/tasks/editDSXMLAdv?PIDnum="+str(PIDnum-1)
    PIDlet['nURL'] = "/tasks/editDSXMLAdv?PIDnum="+str(PIDnum+1)    
    ...
    # return render_template("editDSXMLAdv_index.html",PID=PIDs[PIDnum],PIDnum=PIDnum,len_PIDs=len(PIDs),form=form,raw_xml=raw_xml)
    return render_template("editDSXMLAdv_index.html",PIDlet=PIDlet,PIDnum=PIDnum,form=form,raw_xml=raw_xml)
    



@editDSXMLAdv.route('/editDSXMLAdv/regexConfirm', methods=['POST', 'GET'])
def regexConfirm():
    ...
    return render_template("editDSXMLAdv_regexConfirm.html", return_package=return_package)


def editDSXMLAdv_worker(job_package):
    ...
    # Raw Datastream via Fedora API
    ###############################################################
    raw_xml_URL = "http://%s/fedora/objects/%s/datastreams/MODS/content" % (localConfig.APP_HOST, PID)
    raw_xml = requests.get(raw_xml_URL).text.encode("utf-8")
    ###############################################################
