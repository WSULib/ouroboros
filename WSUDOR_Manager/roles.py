...
class auth(object):
	...
	def __call__(self, f):
		...
		def wrapped_f(*args, **kwargs):
			# logging.debug("Authorized roles for this view:", self.task_roles)

			# if celery context, grab user from job_package and query db
			if self.is_celery:
				username = args[0]['username']
				# logging.debug("celery task initiated by: %s" % username)
				user = models.User.get(username)
				self.user_roles = user.roles()
				# logging.debug("User roles:", self.user_roles)
			...
			if 'admin' in self.user_roles:
				# logging.debug("user is admin, authorized")
				return f(*args, **kwargs)

			# authorize
			role_overlap = set(self.task_roles) & set(self.user_roles)
			if len(role_overlap) > 0:
				# logging.debug("matched on", role_overlap)
				return f(*args, **kwargs)
			else:
				logging.debug("did not find role overlap")
				if not self.is_celery:
