...
def email():
	...
	sender = request.form.get('your_email')
	message = request.form.get('message')
	receiver = request.form.get('recipient_email')
	msg = MIMEMultipart('alternative')
	msg['Subject'] = "WSU Digital Collections"
	msg['From'] = sender
	msg['To'] = receiver
	text = """%s""" % message
	...
	try:
		...
		print "Error: email was not sent to %s." % receiver
...
def login():
	...
	if "WSUDOR" in request.cookies:
		...
		best way to do this?
			...
			- other?
		'''
		r = requests.get('http://192.168.42.5:8000/session_check/%s' % session_id )
