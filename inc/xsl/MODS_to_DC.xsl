...
-->
		...
		<!-- /mods:modsCollection/mods:mods[1]/mods:originInfo[1]/mods:dateIssued[1]
			this was adjacent to dateIssued below...[@point!='start' and @point!='end']-->	
	
		<xsl:template match="mods:originInfo">
			<xsl:apply-templates select="*[@point='start']"/>
			<xsl:for-each
				select="mods:dateIssued |mods:dateCreated[@point!='start' and @point!='end'] | mods:dateCaptured[@point!='start' and @point!='end'] | mods:dateOther[@point!='start' and @point!='end']">
				<dc:date>
					<xsl:value-of select="."/>
				</dc:date>
			</xsl:for-each>
		...
		<xsl:template match="mods:typeOfResource">
			...
			<xsl:if test=".='still image'">
				<dc:type>Image</dc:type> <!--Changed this to "Image" from "StillImage".  This may change collection to collection however...-->
			</xsl:if>
			<xsl:if test=". ='text'">
