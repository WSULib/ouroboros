...
class BagClass(object):
	...
	def __init__(self, object_row, ObjMeta, bag_root_dir, files_location, purge_bags):
		...
		# hardcoded
		self.name = 'collection_shim'  # human readable name, ideally matching filename, for this bag creating class
	...
	def createBag(self):
		...
		# find collection art
		art_files = [ (k,fd[k]) for k in fd.keys() if k == '%s_COLLECTIONART.tif' % self.DMDID ]
