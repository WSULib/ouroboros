...
class BagClass(object):
	...
	def _createBag_physical(self):
		...
		if self.object_row.aem_enriched:
			...
			# get parent, if it's a directory, then set collection parent, else default to collection
			# try:
		...
		if self.object_type == "Item":
			...
			try:
				...
				# Double check it's not an iso file
				# get file extension
				...
				mimetypes.init()
				if file_ext == "iso":
					derived_mime_type = "application/octet-stream"
				else:
					derived_mime_type = fits_identity.attrib['mimetype']
				print "derivedasdfasdf"
				print derived_mime_type
