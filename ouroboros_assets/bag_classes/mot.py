...
class BagClass(object):
	...
	def __init__(self, object_row, ObjMeta, bag_root_dir, files_location, purge_bags):
		# hardcoded
		self.name = 'MOT'  # human readable name, ideally matching filename, for this bag creating class
		self.content_type = 'WSUDOR_Document'  # not required, but easy place to set the WSUDOR_ContentType
	...
	def _makeDatastream(self, each):
		...
		remote_location = "%s/%s" % (self.files_location, filename)
